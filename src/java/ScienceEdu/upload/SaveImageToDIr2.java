 ...
 * @author EKPES
 */
public class SaveImageToDIr2 extends HttpServlet
{
            ...
            throws ServletException, IOException
    {
        try
        {
            ...
            if(session != null)
            {
                ...
                int no_of_pages = (int)session.getAttribute("no_of_pages");
                int id = (int)session.getAttribute("id");
                String course_code = (String)session.getAttribute("course_code");
                String replaceAll = course_code.replaceAll(" ", "_");

                Map<String, ImageFileObject> ifoMap = (Map<String, ImageFileObject>)session.getAttribute("ifoMap");
                List<String> fileNames = ImageProcessor2.createImages(replaceAll, ifoMap, request);
                boolean insert2 = UserDAO.SaveResultImageName(id, fileNames);
                ...
                if(insert2)
                    ...
                    if(count < no_of_pages)
                    {
                        ...
                        String json = gson.toJson(count + " of " + no_of_pages + " Remaining");
