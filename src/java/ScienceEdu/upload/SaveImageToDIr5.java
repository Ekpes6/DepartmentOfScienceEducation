 ...
 * @author EKPES
 */
public class SaveImageToDIr5 extends HttpServlet
{
            ...
            throws ServletException, IOException
    {
        try
        {
            ...
            if(session != null)
            {
                ...
                int id = Integer.parseInt(request.getParameter("nuId"));
                String phone = request.getParameter("nuphone");
                ...
                if(ifoMap != null)
                {
                    ...
                    boolean insert1 = UserDAO.UpdateImageName(id, fileNames);
                    //String imagename = UserDAO.getSavedImageName(id);
                    session.removeAttribute("ifoMap");
                    if(insert1)
                    {
                        Gson gson = new GsonBuilder().setPrettyPrinting().create();
                        String json = gson.toJson("Update of Student data was successfull ");
                    ...
                    {
                        Gson gson = new GsonBuilder().setPrettyPrinting().create();
                        String json = gson.toJson("Update of Student data was not successfull ");
                        try(PrintWriter out = response.getWriter())
                        {
                            out.print(json);
                        }
                ...
                {
                    Gson gson = new GsonBuilder().setPrettyPrinting().create();
                    String json = gson.toJson("NO image to Update ");
                    try(PrintWriter out = response.getWriter())
                    {
                        out.print(json);
                    }
