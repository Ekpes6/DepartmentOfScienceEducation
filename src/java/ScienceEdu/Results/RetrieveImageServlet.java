 ...
 * questions.
 */
package ScienceEdu.Results;

import ScienceEdu.user.DataTableObject3;
import ScienceEdu.user.Result;
            ...
            throws ServletException, IOException
    {
        ...
        try
        {
            ...
            if(session != null)
            {
                ...
                if(!(sessions.equals("Select here") && semester.equals("Select here") && course_code.equals("Select here")
                        && course_title.equals("Select here")))
                {
                    List<Result> SpecificImages = UserDAO.getSpecificImages(sessions, semester, course_code, course_title);
                    if(!SpecificImages.isEmpty())
                    {
                        DataTableObject3 dataTableObject = new DataTableObject3();
                        dataTableObject.setAaData(SpecificImages);
                ...
                {
                    Gson gson = new GsonBuilder().setPrettyPrinting().create();
                    String json = gson.toJson("No valid Entry");
                    try(PrintWriter out = response.getWriter())
                    {
                        out.print(json);
                    }
