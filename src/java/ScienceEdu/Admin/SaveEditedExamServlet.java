 ...
 * @author EKPES
 */
public class SaveEditedExamServlet extends HttpServlet
{
            ...
            throws ServletException, IOException
    {
        ...
        try
        {
            ...
            if(session != null)
            {
                ...
                String full_name = request.getParameter("full_name").toUpperCase().trim();
                String matric_no = request.getParameter("matric_no");
                String course_code = request.getParameter("course_code").toUpperCase().trim();
                String course_title = request.getParameter("course_title").toUpperCase().trim();
                ...
                String semester = request.getParameter("semester").toUpperCase().trim();
                String academic_session = request.getParameter("academic_session").toUpperCase().trim();
                String grade = request.getParameter("grade").toUpperCase().trim();
                String remark = request.getParameter("remark").toUpperCase().trim();

                int nuId = Integer.parseInt(request.getParameter("nuId"));

                boolean insert1 = UserDAO.UpdateExamData(full_name, matric_no, course_code, course_title, credit_hrs, ca, exam_score,
                        total_score, semester, academic_session, grade, remark, nuId);
                if(insert1)
                {
